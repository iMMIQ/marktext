...
describe('CI and release workflow contract', () => {
  ...
  it('uses Windows only for target-native x64 packaging after the Linux gate', () => {
    ...
    expect(windowsJob).toContain('electron-builder build --win nsis zip --x64 --publish never --config.compression=normal')
    expect(windowsJob).toContain('Verify Windows x64 artifact set')
    expect(windowsJob).toContain('build/marktext-x64-win.zip')
    expect(windowsJob).not.toContain('build/*-win.zip')
