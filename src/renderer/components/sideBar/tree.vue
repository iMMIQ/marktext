  ...
  <div class="tree-view">
    ...
    <div class="opened-files">
      <div class="title">
        ...
        <span class="default-cursor text-overflow" @click.stop="toggleOpenedFiles()">{{ $tr('Opened files') }}</span>
        <a href="javascript:;" @click.stop="saveAll(false)" :title="$tr('Save All')">
          <svg class="icon" aria-hidden="true">
            <use xlink:href="#icon-save-all"></use>
          </svg>
        </a>
        <a href="javascript:;" @click.stop="saveAll(true)" :title="$tr('Close All')">
          <svg class="icon" aria-hidden="true">
    ...
    >
      ...
      <div class="tree-wrapper" v-show="showDirectories">
        ...
        <div class="empty-project" v-if="projectTree.files.length === 0 && projectTree.folders.length === 0">
          <span>{{ $tr('Empty project') }}</span>
          <a href="javascript:;" @click.stop="createFile">{{ $tr('Create File') }}</a>
        </div>
      </div>
    </div>
