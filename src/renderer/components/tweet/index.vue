...
export default {
  ...
  methods: {
    ...
    reportViaGithub () {
      this.$nativeApi.shell.openExternal('https://github.com/marktext/marktext/issues/new')
    },
    reportViaTwitter () {
      ...
      this.$nativeApi.shell.openExternal(`${origin}?${Object.keys(params).map(key => `${key}=${params[key]}`).join('&')}`)
